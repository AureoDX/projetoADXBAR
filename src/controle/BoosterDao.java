...
public class BoosterDao {
   ...
   public BoosterBean salvarBooster (BoosterBean boo) throws SQLException{
        
        Connection conn = ConnectionFactory.getConnection();
        PreparedStatement stmt = null;
        try {
            
        stmt = conn.prepareStatement("INSERT INTO Booster (descricao, nome, valor_da_compra, moeda, unidade, data) values(?,?,?,?,?, TO_CHAR(current_date, 'DD-MM-YYYY'))");      
   ...
   // Listar Booster na Tabela:
   
   public List<BoosterBean> CervejaB() throws SQLException{
     Connection conn = ConnectionFactory.getConnection();
     PreparedStatement stmt = null;   
     ResultSet rst = null;    
     
        List<BoosterBean> cervejab = new ArrayList<>();
        try {
         stmt = conn.prepareStatement("SELECT * FROM Booster");
         
         rst = stmt.executeQuery();
            
            while(rst.next()) {
            
            BoosterBean crj = new BoosterBean();
      ...
      //Comando de Deletar da Tabela Booster:
    
      public void delete(BoosterBean dlb) throws SQLException{
        
     Connection conn = ConnectionFactory.getConnection();
     PreparedStatement stmt = null;   
     
        try {
         stmt = conn.prepareStatement("DELETE FROM Booster where id= ? ");
