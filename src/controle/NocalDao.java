...
public class NocalDao {
   
   //Depósito em Nocal: 
    
   public NocalBean salvarNocal (NocalBean boo) throws SQLException{
        
        Connection conn = ConnectionFactory.getConnection();
        PreparedStatement stmt = null;
        try {
            
        stmt = conn.prepareStatement("INSERT INTO nocal (descricao, nome, valor_da_compra, moeda, unidade, data) values(?,?,?,?,?, TO_CHAR(current_date, 'DD-MM-YYYY'))");      
   ...
   // Listar Nocal na Tabela:
   
   public List<NocalBean> CervejaB() throws SQLException{
     Connection conn = ConnectionFactory.getConnection();
     PreparedStatement stmt = null;   
     ResultSet rst = null;    
     
        List<NocalBean> cervejab = new ArrayList<>();
        try {
         stmt = conn.prepareStatement("SELECT * FROM nocal");
         
         rst = stmt.executeQuery();
            
            while(rst.next()) {
            
            NocalBean crj = new NocalBean();
      ...
      //Comando de Deletar da Tabela Nocal:
    
      public void delete(NocalBean dlb) throws SQLException{
        
     Connection conn = ConnectionFactory.getConnection();
     PreparedStatement stmt = null;   
     
        try {
         stmt = conn.prepareStatement("DELETE FROM Nocal where id= ? ");
